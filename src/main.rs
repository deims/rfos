...
use crate::edge_function_rasterizer::*;
...
pub mod render;
pub mod ring_buffer;
pub mod edge_function_rasterizer;
pub mod rasterizer2;
...
impl<'a> Raytracer<'a> {
    pub fn new(scene: Arc<RwLock<Scene>>, config: &'a RenderConfig) -> Self {
        ...
        Raytracer {
            scene,
            config,
            frame: Arc::new(Mutex::new(Texture::new(config.image_width, config.image_height))),
            camera: None,
            tile_queue: Arc::new(Mutex::new(RingBuffer::<Tile>::new(tile_count))),
            tile_width,
            tile_height,
        }
    }

    pub fn render_frame(&mut self, cam: &'a Camera) -> &Arc<Mutex<Texture>> {
        self.camera = Some(cam);
        let mut frame_lock = self.frame.lock().unwrap();
        ...
        for i in 0..self.config.worker_count {
            ...
            let backface_culling = self.config.backface_culling;
                ...
                loop {
                    ...
                    for y in tile.y..tile.y+tile.height {
                        ...
                        for x in tile.x..tile.x+tile.width {
                            ...
                            let res = closest_intersection(&ray, camera.pos, scene.clone(),
                                backface_culling); 
                            if let Some((_ip, model_index, face_index, bc)) = res {
                                let scene_read = scene.read().unwrap();
                                let model = scene_read.get_model(model_index);
                                let material = scene_read.get_material(model.material_index);
                                // let mesh = scene_read.get_mesh(model.mesh_index);
                                ...
                                let mut frag = Fragment::scaled(&trifrags[0], bc[0]);
                                for i in 1..3 {
                                    let tf = Fragment::scaled(&trifrags[i], bc[i]);
                                    frag.add(&tf);
                                }
...
fn get_color(scene: Arc<RwLock<Scene>>, map_index: usize, u: f32, v: f32, default: Vec3) -> Vec3 {
    if map_index == INVALID_INDEX { return default; }
    let scene_read = scene.read().unwrap();
    let tex = scene_read.get_texture(map_index);
    rgba_to_vec3(tex.get_pixel_uv(u, v))
}

fn ambient_color(scene: Arc<RwLock<Scene>>, material: &Material, u: f32, v: f32) -> Vec3 {
    get_color(scene.clone(), material.diffuse_map_index, u, v, vec3![1.0, 1.0, 1.0])
}

fn diffuse_color(scene: Arc<RwLock<Scene>>, material: &Material, u: f32, v: f32) -> Vec3 {
    ...
    cam: &Camera) -> Vec3
{
    ...
    let scene_read = scene.read().unwrap();
    for light in &scene_read.point_lights {
...
fn closest_intersection(
    ...
    campos: Vec3,
    scene: Arc<RwLock<Scene>>,
    backface_culling: bool) -> Option<(Vec3, usize, usize, Vec3)>
{
    let mut min_dist = f32::MAX;
    let mut isect_found = false;
    let mut ip = Vec3::new();
    let mut closest_model_index = 0usize;
    let mut closest_face_index = 0usize;
    let mut closest_bc = Vec3::new();
    let scene_read = scene.read().unwrap();
    for model in &scene_read.models {
        let mesh = scene_read.get_mesh(model.mesh_index);
        let normal_matrix = Mat4::transpose(Mat4::inverse(model.model_matrix));
        for face_index in 0..mesh.faces.len() {
            ...
            if backface_culling {
                ...
                face_normal = Vec3::transform_dir(face_normal, normal_matrix);
                let mut vertex = mesh.vertices[face[0].vertex];
                vertex = Vec3::transform_point(vertex, model.model_matrix);
                if Vec3::dot(vertex - campos, face_normal) >= 0.0 {
                    continue;
                }
            }
            let a = Vec3::transform_point(mesh.vertices[face[0].vertex], model.model_matrix);
            let b = Vec3::transform_point(mesh.vertices[face[1].vertex], model.model_matrix);
            let c = Vec3::transform_point(mesh.vertices[face[2].vertex], model.model_matrix);
            if let Some((tri_point, bc)) = Ray::intersects_triangle(ray, a, b, c) {
                isect_found = true;
                let d = Vec3::dist(ray.origin, tri_point);
                if d < min_dist {
                    min_dist = d;
                    ip = tri_point;
                    closest_model_index = model.index;
    ...
    if isect_found {
        Some((ip, closest_model_index, closest_face_index, closest_bc))
    } else {
        None
    }
}
...
fn main() {
    ...
    // let mut rend = Rasterizer2::new(&scene, &config);
    // _ = rend.render_frame(scene.get_camera(camera_index)).unwrap();

    let scene_rwlock = Arc::new(RwLock::new(scene));
    let mut rend = Raytracer::new(scene_rwlock.clone(), &config);
    let scene_read = scene_rwlock.read().unwrap();
    let frame = rend.render_frame(scene_read.get_camera(camera_index));
