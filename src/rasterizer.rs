...
pub const PRIM_QUEUE_CAP: usize = 8192;
...
impl FragmentProcessor {
    ...
    pub fn launch(
        ...
        config: RenderConfig,
        camera_index: usize,
        receiver: Receiver<RasterPrimitive>) -> thread::JoinHandle<FragmentProcessor>
    {
        thread::spawn(move || {
            let mut fproc = FragmentProcessor::new(scene, config, camera_index, receiver);
            fproc.init();
            fproc.run();
            fproc
        })
